...
public class PlayerController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetKey(KeyCode.W)) moveVertical = 1f;  // Move forward
        if (Input.GetKey(KeyCode.S)) moveVertical = -1f; // Move backward
        if (Input.GetKey(KeyCode.A)) moveHorizontal = -1f; // Move left
        if (Input.GetKey(KeyCode.D)) moveHorizontal = 1f;  // Move right
...
public class PlayerController : MonoBehaviour
{
    public TextMeshProUGUI countText; // TextMeshPro for count
    public TextMeshProUGUI winText;   // TextMeshPro for win message
    private int count;
...
public class ExampleBehaviourScript : MonoBehaviour
{
    public Light sceneLight; // Reference to a Light component
    public AudioSource audioSource; // Reference to an AudioSource component
    public GameObject toggleObject; // Reference to a GameObject to toggle

    void Update()
    {
        // Change light intensity with the "L" key
        if (Input.GetKeyDown(KeyCode.L))
        {
            if (sceneLight != null)
            {
                sceneLight.intensity = sceneLight.intensity == 1f ? 0.5f : 1f; // Toggle between low and high intensity
            }
        }

        // Play a sound with the "P" key
        ...
        // Toggle the active state of a GameObject with the "T" key
        if (Input.GetKeyDown(KeyCode.T))
        {
            if (toggleObject != null)
            {
                toggleObject.SetActive(!toggleObject.activeSelf); // Toggle active state
            }
        }
    }
}
...
// which mouse down? right or left?
using UnityEngine;

public class MouseClick : MonoBehaviour
{
    public float force = 500f; // Force to apply

    void Update()
    {
        // Left mouse button (default mouse click)
    ...
    void OnLeftClick()
    {
        ...
        {
            rb.AddForce(-transform.forward * force); // Apply force forward
    ...
    void OnRightClick()
    {
        if (TryGetComponent<Rigidbody>(out Rigidbody rb))
        {
            rb.AddForce(transform.up * force); // Apply upward force
            rb.useGravity = true; // Enable gravity
        }
    }
}
